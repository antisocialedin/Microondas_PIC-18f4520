...
unsigned char unid=0, dez=0, cent=0, mil=0;
void display (unsigned char u, unsigned char d, unsigned char c, unsigned char m);
char tempo[6];
char cont_tempo=0;
# 32 "Microondas.c"
void main(void)

{
    ...
    PORTAbits.RA5=1;
    lcd_init();
    display(unid, dez, cent, mil);
    lcd_clear();
    lcd_puts("Microondas");
    unid=0, dez=0, cent=0, mil=0;
    ...
    while(1)
    {
        ...
        if(!PORTCbits.RC1)
        {
            while(!PORTCbits.RC1){display(unid, dez, cent, mil);}
            cent++;
            if(cent>9)
            {
                cent=0;
        ...
        if(!PORTCbits.RC2)
        {
            while(!PORTCbits.RC2){display(unid, dez, cent, mil);}
            unid++;
            if(unid>9)
            {
                unid=0;
        ...
        if(!PORTCbits.RC3)
        {
            while(!PORTCbits.RC3){display(unid, dez, cent, mil);}
            TMR0ON=!TMR0ON;
            lcd_clear();
            lcd_puts("Esquentando!");
            display(unid,dez,cent,mil);
        ...
        if(!PORTCbits.RC0)
        {
            while(!PORTCbits.RC0){display(unid, dez, cent, mil);}
            PORTBbits.RB0=0;
            PORTBbits.RB7=!PORTBbits.RB7;
            unid=0;
            dez=0;
...
void __attribute__((picinterrupt("high_priority"))) temp (void)
{
    if(TMR0IF)
    {
        TMR0IF=0;
